...
int main(int argc, char **argv) {
    argparse::ArgumentParser program("quad2tris");

    program.add_argument("input")
        .help("input quad mesh file(s)")
    ...
    for (const auto& input_path : inputs) {
        ...
        if (!out_dir.empty()) {
            output_path = (std::filesystem::path(out_dir) / (p.stem().string() + "_tri.ply")).string();
        } else {
            output_path = (std::filesystem::current_path() / (p.stem().string() + "_tri.ply")).string();
        }
        ...
        try {
            ...
            im.createTrimesh(program.present<int>("-f") ? program.get<int>("-f") : -1);
